  ...
  render: ->
    <div className="tabbed-content" data-side="top" onSubmit={@handleSubmit}>
      ...
      <div className="content-container">
        {switch @state.which
          when 'login-change' then <LoginChangeForm user={@state.user} onSuccess={@props.onSuccess} auth={@props.auth} />
          when 'sign-in' then <SignInForm onSuccess={@onSuccessOrLoginChange} auth={@props.auth} />
          when 'register' then <RegisterForm project={@props.project} onSuccess={@props.onSuccess} auth={@props.auth} />}
      </div>
    </div>
